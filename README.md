...
- `stay-modes.html` - Your Own Stay 모드 소개 상세 페이지
- `styles.css` - 전체 레이아웃, 반응형, 타이포그래피 스타일
- `script.js` - 예약 추천, 인원별 객실 제한, 모바일 메뉴, 룸 이미지 전환, 히어로 영상 제어
...
- 웹사이트에 표시되는 주요 이미지는 로딩 속도를 고려한 JPEG 형식으로 준비되어 있습니다.
- 예약 영역은 휴식 모드와 투숙 인원을 함께 반영해 이용 가능한 객실을 추천합니다.
- 주요 메뉴, 예약 폼, 영상 제어는 키보드로도 이용할 수 있습니다.
